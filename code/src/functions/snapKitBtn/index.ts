import { summaryReview,processReviews } from '../on_work_creation/llm_utils';
import {fetchReviews} from '../on_ticket_extracts/extractTicket';

export async function run(events: any[]) {
    let tags: string = "";
    for (const event of events) {
        tags = event.payload.parameters.trim();
      }
    const token = "eyJhbGciOiJSUzI1NiIsImlzcyI6Imh0dHBzOi8vYXV0aC10b2tlbi5kZXZyZXYuYWkvIiwia2lkIjoic3RzX2tpZF9yc2EiLCJ0eXAiOiJKV1QifQ..RIN5euf91TsoKySuvUybS-sf0SXiRNje10LjxOqE3wL2mMCePhAlajwsfptg5MDVud4kTovqU7CIL2E92kzPDu7PaEqhe6YkjdJ2nm90vgOacH489E7gZmSUALSpMqaTHtxjRckfjAjt60PzyxGBFt3ya-Y-q9jxkpZtxlHiBmCc5DtwYbqqCYJ098YPbuGtUQ2oXW4fbh66DaiYkp_26WaQR5bvzeMoDK0JHKnu2kLuh8lpFiXUVHOd0e9BzKmYlwtSFRiGl_A5_QoYyyOTkWx9NWxPIP0pxS8TUvhcOFdRNgNhn6mN7JNIqhchHikqjUUeivQEfpRS0nhr7XzW6w";
    const snap_in_id= "don:integration:dvrv-us-1:devo/1l0TrbZFtt:snap_in/7d6bbcdd-8fca-4bf5-9ef2-abfdddb19bd5";
    console.log("Creating snap kit to render fetched gif");
    
    const url = "https://api.devrev.ai/timeline-entries.create";
    const review = await fetchReviews(tags)
    
    
    const llmResponse: any = await processReviews(review)
   const txtRes: any = await summaryReview(llmResponse)

    try {
      const resp = await fetch(url, {
        method: "POST",
        headers: {
          authorization: token,
          "content-type": "application/json",
          accept: "application/json, text/plain, /",
        },
        body: JSON.stringify({
          object: "don:core:dvrv-us-1:devo/1l0TrbZFtt:ticket/3045",
          body: "LLM Response",
          type: "timeline_comment",
          snap_kit_body: {
            snap_in_id: snap_in_id,
            snap_in_action_name: "giphy",
            body: {
              snaps: [
                {
                  type: "card",
                  title: {
                    text: "LLM Response!",
                    type: "plain_text",
                  },
                  elements: [
                    {
                      elements: [
                        {
                          text: txtRes.summary,
                          type: "rich_text",
                        },
                      ],
                      type: "content",
                    },
                    {
                      direction: "row",
                      justify: "center",
                      type: "actions",
                      elements: [
                        {
                          action_id: "send",
                          action_type: "remote",
                          style: "primary",
                          type: "button",
                          value: "send",
                          text: {
                            text: "Send",
                            type: "plain_text",
                          },
                        },
                        {
                          action_id: "shuffle",
                          action_type: "remote",
                          style: "primary",
                          type: "button",
                          value: "shuffle",
                          text: {
                            text: "Shuffle",
                            type: "plain_text",
                          },
                        },
                        {
                          action_id: "cancel",
                          action_type: "remote",
                          style: "danger",
                          type: "button",
                          value: "cancel",
                          text: {
                            text: "Cancel",
                            type: "plain_text",
                          },
                        },
                      ],
                    },
                  ],
                },
              ],
            },
          },
        }),
      });
      console.log(resp);
      
  
      if (resp.ok) {
        console.log("Giphy snap kit created successfully");
      } else {
        let body = await resp.text();
        console.log("Error while posting to timeline: ", resp.status, body);
      }
    } catch (error) {
      console.log("Failed to post to timeline: ", error);
    }
    try {
      const resp = await fetch(url, {
        method: "POST",
        headers: {
          authorization: token,
          "content-type": "application/json",
          accept: "application/json, text/plain, /",
        },
        body: JSON.stringify({
          object: "don:core:dvrv-us-1:devo/1l0TrbZFtt:ticket/3045",
          body: "Embedding Response",
          type: "timeline_comment",
          snap_kit_body: {
            snap_in_id: snap_in_id,
            snap_in_action_name: "embedding",
            body: {
              snaps: [
                {
                  type: "card",
                  title: {
                    text: "Embedding!",
                    type: "plain_text",
                  },
                  elements: [
                    {
                      elements: [
                        {
                            alt_text: "Group creation",
                            image_url: "https://raw.githubusercontent.com/abhishek09827/devRevImage/main/Screenshot%202024-02-23%20at%2016.19.12.png",
                            type: "image"
                        },
                      ],
                      type: "content",
                    },
                    {
                      direction: "row",
                      justify: "center",
                      type: "actions",
                      elements: [
                        {
                          action_id: "send",
                          action_type: "remote",
                          style: "primary",
                          type: "button",
                          value: "send",
                          text: {
                            text: "Send",
                            type: "plain_text",
                          },
                        },
                        {
                          action_id: "shuffle",
                          action_type: "remote",
                          style: "primary",
                          type: "button",
                          value: "shuffle",
                          text: {
                            text: "Shuffle",
                            type: "plain_text",
                          },
                        },
                        {
                          action_id: "cancel",
                          action_type: "remote",
                          style: "danger",
                          type: "button",
                          value: "cancel",
                          text: {
                            text: "Cancel",
                            type: "plain_text",
                          },
                        },
                      ],
                    },
                  ],
                },
              ],
            },
          },
        }),
      });
      console.log(resp);
      
  
      if (resp.ok) {
        console.log("Giphy snap kit created successfully");
      } else {
        let body = await resp.text();
        console.log("Error while posting to timeline: ", resp.status, body);
      }
    } catch (error) {
      console.log("Failed to post to timeline: ", error);
    }
  }

export default run;