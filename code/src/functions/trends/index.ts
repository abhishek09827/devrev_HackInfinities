import { processReviews,trendsReview } from '../on_work_creation/llm_utils';
import {fetchData, ticket, fetchReviews,fetchReviewsWithoutTag} from '../on_ticket_extracts/extractTicket';

export async function run(respLlm: string) {
    ...
    const review = await fetchReviewsWithoutTag()
    
    
    const llmResponse: any = await trendsReview(review)
    console.log("LLMResponse",llmResponse.trends);
    try {
